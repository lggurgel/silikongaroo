...
void printUsage() {
    std::cout << "Usage: silikangaroo <public_key_hex> <start_range_hex> <end_range_hex> [num_threads] [--gpu]\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Range: [" << start.get_str(16) << ", " << end.get_str(16) << "]" << std::endl;
    if (useGPU) std::cout << "Mode: GPU Accelerated (Metal)" << std::endl;
    ...
    try {
        ...
        if (kangaroo.isFound()) {
            ...
            std::cout << "Private Key: " << kangaroo.getPrivateKey().get_str(16) << std::endl;

            // Verification is implicit in the collision check, but good to double check.
        } else {
            std::cout << "\n\nSearch finished without finding key (or stopped)." << std::endl;
        }
