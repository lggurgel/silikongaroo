...
public:
    ...
    mpz_class getPrivateKey() const { return privateKey; }
    bool isFound() const { return found; }

    // Statistics
    uint64_t getTotalJumps() const { return totalJumps; }
    double getDuration() const;

    void setUseGPU(bool use) { useGPU = use; }
...
private:
    ...
    // Shared state
    std::unordered_map<std::string, DistinguishedPoint> distinguishedPoints; // Key: compressed point (33 bytes)
    std::mutex mapMutex;
    ...
    // Worker function
    void worker(int id, bool isTame, mpz_class startDist, secp256k1_pubkey startPoint);
    ...
    void processCollision(const std::string& pointHex, const mpz_class& dist, bool isTame);
};
